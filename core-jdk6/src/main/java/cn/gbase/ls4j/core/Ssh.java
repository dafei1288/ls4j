...
import cn.gbase.ls4j.core.internal.GanymedSshService;
...
//    }
    ...
    private Ssh(){
        ...
        sshdService = new GanymedSshService();
...
//        System.out.println(localpath);
        ls.sshdService.uploadSFTP(this.remoteHost,localpath,this.pwd());
        return this;
    }
    ...
    public String pwd() {
        ...
        for (int i = 0; i < this.dirStack.size(); i++) {
            sb.append( this.dirStack.get(i)+PATHSPILTER);
        }

        String path  = sb.toString().endsWith(PATHSPILTER)?sb.toString().substring(0,sb.toString().length()-1):sb.toString();//this.dirStack.stream().collect(Collectors.joining(PATHSPILTER));
        String res = path;
...
//        String cli = this.getDir().replace(this.getPlaceholder(),path);
    ...
    public List<String> ls(String path) throws Exception {
        String cli = this.dir.replace(this.getPlaceholder(),path);
//        String cli = this.mkdir.replace(this.getPlaceholder(),path);
        return ls.sshdService.lsFiles(remoteHost,cli,this.chartset);
    }
...
//              res = this.jschService.pwd(remoteHost,path,this.chartset);
    ...
    public Ssh rmdir(String remotepath) throws Exception {
        this.sshdService.rmdir(this.remoteHost,remotepath,this.chartset);
        return this;
    }

    public Ssh printLs(){
        try {
            List<String> al = this.ls();
